-- Migration: Update Credit Consumption to 2 Credits (Fixed Version)
-- Description: Updates credit consumption functions to deduct 2 credits per analysis instead of 1
-- This version includes DROP statements to handle existing functions

-- Drop existing functions first to avoid return type conflicts
DROP FUNCTION IF EXISTS deduct_credit_on_completion(UUID, UUID);
DROP FUNCTION IF EXISTS consume_user_credit(UUID, UUID);
DROP FUNCTION IF EXISTS consume_credits_for_analysis(UUID, UUID, INTEGER);
